    ...
    *conto = 0;                        // p[0]=0;
    ...
    if (fork() == 0) { // primo figlio
        for (; versamento > 0; versamento--) {
            // (*conto)++;  // provate a rimpiazzare le 3 istruzioni successive
            // con questa singola operazione (comunque non atomica)
            temp = *conto;
...
// questo codice, su un sistema multi-core, può essere forzato a girare su una
// sola CPU con un comando del tipo: 
//   taskset --cpu-list 1 ./concproblem
